classdef PropertiesDetails_LUT <  handle
    properties (SetObservable = true)
        Name = 'Great Cambourne';
        Available
        Emailed = 'yes';
        EmailedReply = 'no';
        DateAvailablityLastChecked
        PlotArea = 240; %m^2
        Dimensions_Meters = [NaN,NaN]; %m
        Price = 146000;
        NoOfProperty
        Distance = [];
        SalePrice
        PlanningPermissionDoc
        PlotLegalType = [];
        ContactName = [];
        WebLink = '';
        PlanningPermission = 'unknown';
        Postcode = '';
        GoogleMap
        WebSite
        Services_MainGas
        ...
        DemolishingRequired
        AddtionalRequiredCosts = 0;
    end
    properties (SetObservable = true) %READ ONLY OUTPUTS
        BuildCost = NaN;
        ProjectCost = NaN;
        Profit = NaN;
        Margin_perc = NaN;
    end
    properties (Hidden = true) %Rules
        OBJ_BC
        OBJ_BC_Adj
    ...
    properties (Hidden = true, SetObservable = true)
        BuildCost_BuildRoute_LUT = {    'A'; ...
                                        ...
                                        'C'; ...
                                        'D'};
        Name_LUT = {    'At 24 Hereward Close'; ...
						'Rampton, Cambridge'; ...
                        'Long Lane - Willingham - Cambridge'; ...
    ...
    methods
        ...
        function RUN(varargin)
           %%
            obj = varargin{1};
            LandDetails = obj.property_LUT(obj.Name);
            NAMES = fieldnames(LandDetails);
            x = size(NAMES,1);
            for i = 2:x
                 obj.(NAMES{i}) = LandDetails.(NAMES{i});
            end            
        end
    end
    methods (Hidden = true)
        function obj = PropertiesDetails_LUT()
        end
        function LandDetails = property_LUT(obj,PropertyName)
            switch PropertyName
